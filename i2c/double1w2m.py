...
for line in f:
   ...
   n+=1
   x.append(ParsedTime+3600) #+3600 in summer time order to get it in GMT+1
...
for line in f2:
   ...
   ParsedTime = time.mktime(time.strptime(linespit[0], "%a %b %d %H-%M-%S %Y"))
   n+=1
   x2.append(ParsedTime+3600) #+3600 in summer time order to get it in GMT+1
...
ax.set_ylim([10,30])
ax.set_xlim([datetime.datetime.now()-datetime.timedelta(days=7),datetime.datetime.now()])
ax.xaxis.set_major_locator(pylab.matplotlib.dates.DayLocator())
ax.xaxis.set_major_formatter(pylab.matplotlib.dates.DateFormatter('%a'))
ax.xaxis.set_minor_locator(pylab.matplotlib.dates.HourLocator())
ax.grid(True)
ax.set_ylabel('Temperature (C)', fontsize=30)
ax.set_xlabel('Time', fontsize=30)
ax.set_title('One Week', fontsize=30)
...
pylab.plot_date(t2,y2,"--ro",None ,True, False)
ax.set_ylim([10,30])
ax.set_xlim([datetime.datetime.now()-datetime.timedelta(days=60),datetime.datetime.now()])
ax.xaxis.set_major_locator(pylab.matplotlib.dates.DayLocator())
ax.xaxis.set_major_formatter(pylab.matplotlib.dates.DateFormatter('%d'))
ax.xaxis.set_minor_locator(pylab.matplotlib.dates.HourLocator())
ax.grid(True)
ax.set_ylabel('Temperature (C)', fontsize=30)
ax.set_xlabel('Time', fontsize=30)
ax.set_title('2 Month', fontsize=30)
pylab.xticks(rotation=15)
...
fig.savefig("/var/www/tempall1w2m.png",dpi=72)
